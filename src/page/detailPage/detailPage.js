...
const generateCommentSection = (comment) => {
    const generateCommentSectionHeader = (comment) =>{
        ...
        const mouseupCancel = () =>{
            ...
            if (comment == null) {
                ...
                if (comment.lat && comment.lng) {
                    $("#detailCommentInputLatLng").val(convertDec2DMS(comment.lat, comment.lng));
                } else
        ...
        const mouseupDelete = () =>{
            ...
            if (check) {
                ...
                $("#detailCommentCacheState").prop('disabled', true);
                detailFinalCacheState.options.selectedIndex = $("#detailCommentCacheState").val();
                $('#gccommentEditor').hide();
                $('#gccommentEditor').hide();
                $('#commentCommandAdd').show();
                $('#commentCommandEdit').hide();
        ...
        return html`
            <p>
                ...
                <a id="commentCommandAdd" style="display: none;" onmouseup=${mouseupAdd}>
                    <img src="${commentIconAdd}" title="${lang.detail_add}" style="cursor:pointer">
                </a> 
                <a id="commentCommandEdit" style="display: inline;" onmouseup=${mouseupEdit}>
                    <img src="${commentIconEdit}" title="${lang.detail_edit}" style="cursor:pointer">
                </a>
                <a id="commentCommandShare" style="display: inline;" onmouseup=${mouseupShare}>
                    <img src="${commentIconShare}" title="${lang.detail_share}" style="cursor:pointer">
                </a>
                <a id="commentCommandCancel" style="display: none;" onmouseup=${mouseupCancel}>
                    <img src="${commentIconEditCancel}" title="${lang.detail_cancel}" style="cursor:pointer">
                </a>
                <a id="commentCommandSave" style="display: none;" onmouseup=${mouseupSave}>
                    <img src="${commentIconSave}" title="${lang.detail_save}" style="cursor:pointer">
                </a>
                <a id="commentCommandFinalSave" style="display: none;" onmouseup=${mouseupFinalSave}>
                    <img src="${commentIconSave}" title="${lang.detail_finalsave}" style="cursor:pointer">
                </a>
                <a id="commentFinalDelete" style="display: none;" onmouseup=${mouseupFinalDelete}>
                    <img src="${deleteMysteryIcon}" title="${lang.detail_finaldelete}" style="cursor:pointer">
                </a>
                <a id="commentCommandJump" style="display: none;" href="#gccommentarea">
                    <img src="${commentIcon}" title="${lang.detail_jumptocomment}" style="cursor:pointer">
                </a>
                <a id="commentCommandArchive" style="display: none;" onmouseup=${mouseupArchive}>
                    <img src="${currentlyArchived?archiveRemove:archiveAdd}" title="${currentlyArchived?lang.table_removefromarchive:lang.table_addtoarchive}" style="cursor:pointer">
                </a>
                <a id="commentCommandDelete" style="display: none;" onmouseup=${mouseupDelete}>
                    <img src="${commentIconDelete}" title="${lang.detail_delete}" style="cursor:pointer">
                </a>
                ...
                <small>last saved: ${lang.detail_lastsaved + ": " + lastSaveTime}</small>
                <br>
                State:
                ...
                Final:
                <input id="detailCommentInputLatLng" value=${(comment.lat && comment.lng)?convertDec2DMS(comment.lat, comment.lng):DEFAULTCOORDS} style="margin-left:5px; margin-right:5px; height:35px;" disabled="true" size="30"></input>
    ...
    return html`
        ${generateCommentSectionHeader(comment)}
    `;
};
...
const getGUID = () => {
    const url = document.getElementById('ctl00_ContentBody_lnkPrintFriendly').getAttribute('href');
...
const getCachename = () => {
        return unescapeXML(trim(document.getElementById('ctl00_ContentBody_CacheName').innerHTML));  
};

const getCachecode = () => {
    return trim(document.getElementById('ctl00_ContentBody_CoordInfoLinkControl1_uxCoordInfoCode').innerHTML);  
};

const retrieveOriginalCoordinates = () => {
    var origCoordinates;
    // try to get it from GS
    if (window.userDefinedCoords && window.userDefinedCoords.data
            && window.userDefinedCoords.data.oldLatLngDisplay) {
        origCoordinates = parseCoordinates(window.userDefinedCoords.data.oldLatLngDisplay);
...
export const gccommentOnDetailpage = () => {
    ...
    if (hookForCommentSection.length > 0){        
        hookForCommentSection.parent().before('<div id="gccommentarea" name="mycomments"><div>');        
    ...
    viewer_instance = createViewer($("#gccommentViewer")[0], comment.commentValue);
    editor_instance = createEditor($("#gccommentEditor")[0], comment.commentValue);
