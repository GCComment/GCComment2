...
export const generateDisplayFilters = () => {
    ...
    return html`
        <div id="displayFilters" style="display:none;">
            ...
            <select
                id="archivedSelector"
                style="margin-left:5px;background: none;"
                onchange=${onArchivedSelectorChanged}
            >
                <option
                    id="archivedSelector_no_archived"
                    selected="${selectedArchivedFilter === "no_archived"}"
                >
                    ${lang.archived_filter_no_archived}
                </option>
                <option
                    id="archivedSelector_include_archived"
                    selected="${selectedArchivedFilter === "include_archived"}"
                >
                    ${lang.archived_filter_include_archived}
                </option>
                <option
                    id="archivedSelector_only_archived"
                    selected="${selectedArchivedFilter === "only_archived"}"
                >
...
export const generateTableDiv = () => {
    return html`
        <div
            id="gccommenttablediv"
            style="display:none; margin: 5px; padding: 4px; outline: 1px solid rgb(215, 215, 215); position: relative; background-color: rgb(235, 236, 237);"
        />
    `;
};
...
const generateTableRow = (
    ...
    const getStateIcon = (state) => {
        ...
        if (state == StateEnum.found) return state_found;
    ...
    function changeState(event) {
        ...
        if (
            (filter != null && targetState != filter && filter != ARCHIVED) ||
            (filter == null && targetState != null)
        ) {
            var row = $(event.target).parents("tr");
            $("#gccommentoverviewtable")
                .DataTable()
                .row(row[0])
                .remove()
                .draw();
    ...
    return html`
        <tr role="row" class="${rowCount % 2 == 0 ? "even" : "odd"}">
            <td class="sorting_1">
                <img
                    class="tableStateIcon"
                    src="${getStateIcon(comment.state)}"
                />
                <a
                    href="${"http://www.geocaching.com/seek/cache_details.aspx?guid=" +
                    comment.guid}"
                >
                    ${comment.name}
                </a>
            ...
            <td style="width: 505px;">${createCachePrintout(comment)}</td>
            <td>
                <small> ${createTimeString(comment.saveTime)} </small>
            </td>
            <td>${createActionButtons(comment)}</td>
        </tr>
    `;
};
...
export const refreshTableDiv = (show) => {
    const commentKeys = GCC_listValues().filter(
        (key) => key.indexOf(COMPREFIX) != -1
    );
    /**
     * @type {Array<CacheComment>}
     */
    var comments = commentKeys.map((key) =>
        doLoadCommentFromGUID(key.substr(COMPREFIX.length))
    );
    ...
    render(
        ...
        html`
            <table
                ...
                role="grid"
                aria-describedby="gccommentoverviewtable_info"
            >
                <thead>
                    <tr role="row">
                        <th
                            ...
                            class="sorting_asc"
                            tabindex="0"
                            aria-controls="gccommentoverviewtable"
                            rowspan="1"
                            colspan="1"
                            aria-sort="ascending"
                            aria-label="Cache: activate to sort column descending"
                        >
                            Cache
                        </th>
                        <th
                            style="font-weight: bold; width: 505px;"
                            class="sorting"
                            tabindex="0"
                            aria-controls="gccommentoverviewtable"
                            rowspan="1"
                            colspan="1"
                            aria-label="My comment &amp;amp; final coordinates: activate to sort column ascending"
                        >
                            My comment &amp; final coordinates
                        </th>
                        <th
                            ...
                            rowspan="1"
                            colspan="1"
                            aria-label="Last save: activate to sort column ascending"
                        >
                            Last save
                        </th>
                        <th
                            style="width: 72px; font-weight: bold;"
    ...
    $("#gccommentoverviewtable_length")
        .append(
            $("#gccommentoverviewtable_filter > label").css("margin", "10px")
        )
        .css("padding", "5px")
        .css("font-weight", "500")
        .append($("#gccommentoverviewtable_paginate"))
        .append($("#gccommentoverviewtable_info"))
        .append(
            "<span>" +
                lang.table_filtered_by +
                " " +
                filteredByString +
                "</span>"
        )
