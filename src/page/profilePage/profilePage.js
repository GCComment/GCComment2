    ...
    return render(
        ...
        html`
            ...
            >
                ...
                ${generateConfigDiv()} ${generateTableDiv()}
                ${generateExportDiv()} ${generateImportDiv()}
                ${generatePatchDiv()} ${generateDeleteAllDiv()}
            </div>
        `
    );
};
...
const onConfigDivButtonMouseover = (evt) => {
    ...
    var stats = html`
        <p style="font-size:13px;">
            ...
            ${getNumberOfComments()} (${GCC_getValue("countWhite")}
            ${lang.type_untyped}, ${GCC_getValue("countRed")}
            ${lang.type_unsolved}, ${GCC_getValue("countGreen")}
            ${lang.type_solved}, ${lang.and} ${GCC_getValue("countGray")}
            ${lang.type_found})
            <br />
            <b>${lang.ov_amountarchive}</b> ${GCC_getValue("countArchive")}
            <br />
            <b>${lang.ov_lastim}: </b> ${lastim
                ? createTimeString(lastim)
                : lang.never}
            <br />
            <b>${lang.ov_lastex}: </b> ${lastex
                ? createTimeString(lastex)
                : lang.never}
            <br />
            <b>${lang.ov_lastup}: </b> ${createTimeString(
                parseInt(GCC_getValue("updateDate"))
            )}
        </p>
    `;
...
export const gccommentOnProfilePage = (newGcDesign = false) => {
    ...
    appendCSS(
        "text",
        ".odd{background-color:#ffffff} .even{background-color:#E8E8E8}" +
            ".ui-icon{display:inline-block;}" +
            " .tableStateIcon{width: 11px;margin-right:3px}" +
    ...
    appendCSS(
        "text",
        ".tableFinal, .tableComment, .tableWaypoints{margin: 0px;} .tableComment{font-family:monospace;font-size:small; white-space: pre-line; word-wrap: break-word;} .tableWaypoints{width: 100%}"
    );
    appendCSS(
        "text",
        ".tableGCComment {text-transform:none; font-size:small;} label{font-size: small; font-weight:400;text-transform:none;display:initial;margin-bottom:4px;max-width:100%} select{display:initial;font-size: small; background: none; width:auto;padding:initial;-moz-appearance:listbox;-webkit-appearance:listbox;}"
    );
