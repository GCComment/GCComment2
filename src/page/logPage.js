...
export const gccommentOnLogPage = function () {
    if (("" + window.location).indexOf('LUID=') >= 0) {
        ...
        var guid = $('#ctl00_ContentBody_LogBookPanel1_WaypointLink')[0].getAttribute('href')
            .split("guid=")[1];
        if (guid) {
            ...
            if (comment) {
                var submitButton = $('#ctl00_ContentBody_LogBookPanel1_btnSubmitLog')[0];
                ...
                $(generateGccActionDiv()).insertBefore(submitButton).append(submitButton);
