...
export const appendCheckBox = function(id, label, extrafunction, floatLeft, imageURL) {
    ...
    return html`
        <div id="div${id}" style="${floatLeft?'float:left':''}">
            <input id="${id}" type="checkbox" checked=${checked} class="Checkbox" onmouseup=${updateValue} style="margin:3px"/>
            ${label?html`
                <label id="${id}_label" for=${id} onmouseup=${updateValue}>
                    ${imageURL?html`
                        <img src="${imageURL}" style="margin-right:3px;width:12px;"/>
                    `:null}
                    ${label}
                </label>
            `:null}
